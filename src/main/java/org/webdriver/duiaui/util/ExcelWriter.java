...
public class ExcelWriter {
        ...
        public static void writeToExcel(String fileDir,String sheetName,List<Map> mapList) throws Exception{
            ...
            try {
                ...
                if(titleRow!=null){
                    for(int rowId=1;rowId <= mapList.get(0).size();rowId++){
                        ...
                        for (short columnIndex = 0; columnIndex < columnCount; columnIndex++) {  //遍历表头
                            ...
                            cell.setCellValue(mapList.get(columnIndex).get(rowId)==null?null:mapList.get(columnIndex).get(rowId).toString());
    ...
    public static void writeToExcelMapKey(String fileDir,String sheetName,List<Map> mapList,int i) throws Exception{
        //创建workbook
        //File file = new File(fileDir);
        try {
        ...
        try {
            ...
            if(titleRow!=null){
                for(int rowId=0;rowId<mapList.size();rowId++){
                    Map map = mapList.get(rowId);
                    HSSFRow newRow=sheet.createRow(rowId+i+1);
                    //横着写
                    for (short columnIndex = 0; columnIndex < columnCount; columnIndex++) {  //遍历表头
               /*         HSSFCell cell = newRow.createCell(columnIndex);
                        cell.setCellValue(mapList.get(columnIndex).get(rowId)==null?null:mapList.get(columnIndex).get(rowId).toString());*/
